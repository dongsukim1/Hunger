...
This project aims to address that flaw by replacing global rankings with contextual, user-defined groupings in which ratings are locally meaningful. The system is designed to minimize external API usage, store only necessary data, and generate high-quality feedback signals for future machine learning–based recommendations.

Scope and Non-Goals
This is a personal project intended to demonstrate backend and ML engineering skills, not a production competitor to Beli. The system will support a single user, limited geographic scope (e.g., a portion of the Bay Area), and one-time ingestion of third-party data. Social features, real-time synchronization, complex frontend interactions, and multi-user support are explicitly out of scope. The focus is on correctness, clarity, modularity, and extensibility rather than scale or polish.
...
The ML component is intentionally deferred but enabled by design. Because ratings are contextual, labeled, and explicit, the resulting dataset is significantly cleaner than typical restaurant recommendation data. Features may include list context, price level, distance, and user history within similar lists.
...
Conclusion
This design replaces global restaurant rankings with contextual, user-defined comparisons that scale naturally with user history. It minimizes external API usage, avoids unnecessary data ingestion, and produces high-quality signals for machine learning. The system is intentionally simple, modular, and aligned with real user behavior, addressing a concrete usability flaw in existing applications while remaining technically disciplined and extensible.

As a personal project, it prioritizes demonstrable engineering rigor over breadth of features, enabling a clean, functional MVP that showcases backend and ML pipeline design.
