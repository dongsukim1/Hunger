...
This project aims to address the first flaw by replacing global rankings with contextual, user-defined groupings in which ratings are locally meaningful even as the number of locations scales. A minimalist app design was implemented to address the second, a shift from a social media/competitive eating platform. As groupings begin to lose meaning or become unmanageably large, the user can choose to create new stratifications that make sense to them. The system is currently designed to minimize external API usage, store only necessary data, and generate high-quality feedback signals to fuel machine learning-based recommendations.
</details>
...
#### Backend
Navigate to the main project directory, run the following, and open up http://localhost:8000
```bash
...
This was a learning project built in under ten days. Recommendations are still constrained by synthetic cold-start data quality, and feature migration to canonical sources is currently in progress.
...
- The initial ingestion is done via the Google Places API where we seed the local database with its geospatial queries (Nearby Search). They are conservative and overlapping because querying > 20 locations causes truncated lists and we lose everything below the cutoff.
...
- Results are deduplicated via unique google_place_id and written into the local database. After this step, no live Google API calls are intended. All user interactions, searches, and recommendations query the local database directly.
- Restaurants now have canonical feature rows in `restaurant_features` (backfilled from current sources), with `synthetic_attributes` retained as a transitional source during migration.

  Flow of Data
  - Ingestion: A script calls `fetch_places_nearby` and results are deduplicated + committed to the local DB.
  ...
  - Interactive Loop: The system currently defaults to ML-based question selection -> User answers -> Candidates are filtered -> This repeats until <=3 candidates remain.
...
- The project relied heavily on synthetic data to train the model and bootstrap recommendations. This was necessary to solve cold-start issues while no real userbase existed. For real use, synthetic fallback paths are intended to be phased out.
</details>
...
The system supports an explicit recommendation action initiated by the user through a guided questionnaire. The user answers a small adjustable number of discrete questions (e.g., cuisine preference, maximum distance, preferred price) to define a context. The engine selects candidate restaurants matching these constraints and produces a predicted preference score. The system then recommends an adjustable number (e.g., three) of options. More options means fewer questions required. Crucially, every recommendation is followed by explicit user feedback when the user visits and rates the restaurant within the inferred or selected context. Because ratings are contextual, labeled, and explicit, the resulting dataset should be cleaner. Features include contextual signals and restaurant attributes. A simple baseline heuristic question selector still exists, but the default runtime path currently uses ML-based selection.

<details>
  <summary>Technical Implementation</summary>
  
The ML component is built as a probabilistic recommendation layer on top of the local database.
- Model Core: Uses XGBoost to predict a user's likely rating (1.0-5.0) for a restaurant based on its features and the current context.
- Predictive Features: The model is trained on contextual triplets:
  - Restaurant Attributes: Price tier, cuisine (one-hot encoded), and amenity/ambiance booleans from canonical `restaurant_features` (currently backfilled during migration).
  - Contextual Data: Context is normalized into a canonical context set (e.g., Date Night, Group Hang, Quick Lunch, Weekend Brunch, Late Night Eats) and one-hot encoded.
- Evaluation: Training uses a grouped held-out split by feature-signature hash to reduce duplicate-row leakage and reports MAE/AUC plus ranking metrics (NDCG@k and hit-rate@k).
- Deployment: The trained model is serialized into `rating_model.json`, and the training feature schema is saved to `rating_model_features.json`. The FastAPI backend loads these artifacts for real-time inference and can reload updated artifacts after retrain.

The XGBoost model was trained by creating semi-realistic synthetic user personas with semi-random preferences. A session simulator was created so these personas could generate feedback for recommendations, and noise was intentionally introduced to reflect human responses. Removing user IDs from model features was necessary to avoid leakage and memorization.
</details>
...
```bash
python -m data.backfill_restaurant_features
```
- Validate canonical coverage and nulls:
```bash
python -m data.validate_restaurant_features
```
- Feature loading mode toggle:
  - `HUNGER_FEATURE_MODE=canonical+fallback` (default)
  - `HUNGER_FEATURE_MODE=canonical-only`
...
The implementation of very basic social features to allow adding people you know and being able to look at lists that they have publicly available (similar to Spotify). This project avoided designing a social media platform the way Beli seems to be developing. I think people might have mixed opinions about this but some users might not really care what other people think. Individual palates are very different. As it stands, sharing lists with other users and adding more restaurants is possible but user unfriendly. You could share personal databases with other users or modify the code directly to poll the Google Places API using your own API key and obtain more locations but none of this is available internally. More UX features would naturally follow social features such as being able to hide lists, ratings, or mix restaurants with no ratings into normal lists.

The implementation of a basic interactive map and integrating simple geospatial analysis into the machine learning components. I imagine that this would be very involved, require rehauling data collection, and be expensive to maintain.
